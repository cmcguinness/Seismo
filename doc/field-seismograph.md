...
| **no record button** | the piezo edge IS the trigger: the rig stays armed and captures a window per blow. The T-Display-S3's onboard user button (GPIO 14) drops a "new line" marker if wanted | offsets come from blow order + the notebook, as with commercial gear |
...
- Onboard user button (GPIO 14): press = write a "line marker" record, so a survey's
  blows group themselves. No recording state to manage; the rig is always armed.
...
2. Log triggered windows to internal flash (tap the piezo by hand); read them over USB on the Mac. **This is the milestone.** (SD only if continuous logging is ever wanted.)
