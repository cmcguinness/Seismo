...
| 1 | **180 Ω** axial | PhotoMOS LED from a 3 V pin. **Was 330 Ω** — see `calibrator-build.md`, "The LED resistor the BOM got wrong": ATtiny85 `V_OH` is 2.5 V min at −5 mA / V_CC = 3 V, and the AQY212EH is only guaranteed to turn on up to 3.0 mA, so 330 Ω on an aged cell gives 2.6 mA and the box stops calibrating as its cell ages |
...
| 1 | **180 Ω** axial | its LED drive, same as the injector's — and 180 Ω for the same reason |
