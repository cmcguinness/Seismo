...
| `apps02.mcguinness.ai` (public VPS) | the **public dashboard**, https://seismo.mcguinness.ai — same image, fed **outbound-only** by pi5 (rsync every minute + the live ring every 3 s). Nothing at the house is reachable from the internet. Also the **visitor digest** (nginx log → Dokku Postgres → ntfy daily; code lives in the private sibling repo `Seismo-private`) | `./deploy.sh public` |
