...
- **Gen-1 geophone case (2026-07-28) — modeled, not yet printed.** `parts/geophone_case.py` + `parts/geophone_case_lid.py`. A deliberately crude POC whose only jobs are *pick it up and set it down* and *put the XLR on the case so the cable unplugs*. **116 × 116 × 79 mm** (74 body + 5 lid, plus ~4 mm of foot screw) rounded square, PLA, no seals, no ballast, no inserts. 30 mm of headroom over the element for terminals, wire and slack. Lid engraved "GEOPHONE". Three screw heads as feet (three-point contact; also lets the floor print flat on the bed instead of bridging over printed feet; ~1.4° of tilt per turn, which is the only leveling it has and enough). **No vents** — no heat source inside and the cavity's lowest acoustic mode is ~1.5 kHz, 30× above Nyquist, so they bought nothing and cost a convection path over the element. **XLR connector measured (Charles, 2026-07-28):** 22 mm shell → **23 mm bore**; flange 30 × 25 mm; two 5 mm holes **on the flange diagonal, 30 mm centre-to-centre** → offsets ±(11.52, 9.60) mm from the bore centre, clearing the bore by 1.80 mm. **Which diagonal is still open** — the pattern is symmetric under 180° rotation but not under mirroring, so `parts/xlr_coupon.py` carries both (4 holes, the "A" pair dimpled). Set `xlr_screw_diagonal` in `dimensions.py` to `"A"`/`"B"` once the connector has sat on the coupon and the case drills just that pair; until then the case ships bore-only. **Print the coupon first** — 60 × 52 × 3 mm, ~15 min, reproduces the wall cross-section exactly (3 mm wall, 34 × 34 relief → 2.4 mm panel) so it also proves the 1–3 mm panel range and that a nut seats behind. **Fasteners: 7 × #6 × ½″ sheet-metal (4 lid, 3 feet) + 2 × M3 × 12 + washers + M3 nuts for the XLR** — 2.4 mm of PLA will not hold a self-tapper against the latch pull, and M3 needs a washer in the connector's 5 mm holes. Fit the connector before the geophone goes in. Views: `doc/geophone_case*.svg`.
