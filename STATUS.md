...
- **Gen-1 geophone case (2026-07-28) — modeled, not yet printed.** `parts/geophone_case.py` + `parts/geophone_case_lid.py`. A deliberately crude POC whose only jobs are *pick it up and set it down* and *put the XLR on the case so the cable unplugs*. **116 × 116 × 79 mm** (74 body + 5 lid, plus ~4 mm of foot screw) rounded square, PLA, no seals, no ballast, no inserts. 30 mm of headroom over the element for terminals, wire and slack. Lid engraved "GEOPHONE". Three screw heads as feet (three-point contact; also lets the floor print flat on the bed instead of bridging over printed feet; ~1.4° of tilt per turn, which is the only leveling it has and enough). **No vents** — no heat source inside and the cavity's lowest acoustic mode is ~1.5 kHz, 30× above Nyquist, so they bought nothing and cost a convection path over the element. The XLR **24 mm bore is modeled but its two flange holes are not** — spacing unconfirmed, so use the connector itself as the drill jig (3.2 mm, sharp bit, light pressure) rather than risk scrapping the print. +Y wall locally thinned to 2.4 mm (D-series accepts a 1–3 mm panel), which also gives the nuts a flat 34 mm landing inside. **Fasteners: 7 × #6 × ½″ sheet-metal (4 lid, 3 feet) + 2 × M3 × 10 countersunk + 2 × M3 nuts for the XLR** — the flange is countersunk and 2.4 mm of PLA will not hold a self-tapper against the latch pull. Fit the connector before the geophone goes in. Views: `doc/geophone_case*.svg`.
