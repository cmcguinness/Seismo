...
- **Gen-1 geophone case (2026-07-28) — modeled, not yet printed.** `parts/geophone_case.py` + `parts/geophone_case_lid.py`. A deliberately crude POC whose only jobs are *pick it up and set it down* and *put the XLR on the case so the cable unplugs*. **116 × 116 × 79 mm** (74 body + 5 lid, plus ~4 mm of foot screw) rounded square, PLA, no seals, no ballast, no inserts. 30 mm of headroom over the element for terminals, wire and slack. Lid engraved "GEOPHONE". Three screw heads as feet (three-point contact; also lets the floor print flat on the bed instead of bridging over printed feet; ~1.4° of tilt per turn, which is the only leveling it has and enough). **No vents** — no heat source inside and the cavity's lowest acoustic mode is ~1.5 kHz, 30× above Nyquist, so they bought nothing and cost a convection path over the element. **XLR connector (measured 2026-07-28 + published pattern):** 22 mm shell → **23 mm bore**; flange 30 × 25 mm; two countersunk holes **30 mm centre-to-centre on a diagonal**. The offsets are the published D-series pattern — **±(10, 11.5) mm** from the bore centre, larger offset on the flange's 30 mm axis — which spans 30.5 mm and so reproduces the measurement. (An earlier attempt to derive them from the flange's *corner* diagonal gave ±(11.52, 9.60), dx/dy the wrong way round, leaving 0.4 mm of flange; the flange is not square, so those are different lines.) Panel holes are 5 mm as measured — a clearance hole cannot be too large, and 5 mm passes M3-with-washer or M4 whether the flange's 5 mm is a through hole or an M3 countersink. **Only V-vs-H is open**: which way the 30 mm flange axis runs on the wall. `parts/xlr_coupon.py` is a 104 × 56 × 3 mm two-station test card (~20 min) carrying both, each with all four sign combinations so handedness never has to be established — two holes take screws, two hide under the flange. Set `xlr_flange_axis` to `"V"`/`"H"` and the case cuts the same four; both paths dry-run verified. The card reproduces the wall cross-section exactly (3 mm wall, 34 × 34 relief → 2.4 mm panel), so it also proves the 1–3 mm panel range, that a nut seats behind, and that the 1.24 mm web between hole and bore survives printing. **Fasteners: 7 × #6 × ½″ sheet-metal (4 lid, 3 feet) + 2 × M4 × 12 + washers + nuts for the XLR** — 2.4 mm of PLA will not hold a self-tapper against the latch pull. Fit the connector before the geophone goes in. Views: `doc/geophone_case*.svg`.
