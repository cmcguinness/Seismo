...
1. **Calibration injector — parts are IN HAND (09-21); the schematic, the build doc and
   the box are done.** `doc/calibrator-build.md` is what you solder from:
   `doc/calibrator.svg` (schemdraw, generated), board layout, build order, flashing,
   bring-up. The box is `parts/calibrator_case.py` + `calibrator_lid.py`, with panel
   elevations in `parts/calibrator_case.png`. **Remaining: solder it, flash it, print
   the box, acceptance-test, then the firmware's 48 h soak.** Two things to settle at
   the bench — confirm the panel button's thread (12 mm assumed) before printing, and
   the button paths (short = restart soak, long = fire) are still the only untested
   logic. Everything on the software side is built and tested: `calfinder.py` (0 false positives over
...
Parts are in hand, so the order was schematic first, box second — the box's interior
comes out of the finished board anyway.

**`doc/calibrator.svg`**, generated by `doc/calibrator_schematic.py` the same way
`rev2-frontend.svg` is, so it cannot drift from its description. The drawing's one job
is the dashed red isolation barrier: control left, coil right, and the only things
crossing are the two PhotoMOS, with 5 kV and no galvanic path. **`doc/calibrator-build.md`**
carries it, plus board layout, solder order with a check at every step, flashing, and
bring-up.

**Both pinouts were read off the datasheets rather than recalled**, because this is a
drawing somebody solders from. Worth it: the LM4040's TO-92 drawing in SNOS633K is a
**bottom** view, which is exactly how that part gets fitted backwards.

**Two corrections came out of doing the arithmetic:**

- **R1/R2 are 180 Ω, not the BOM's 330 Ω.** ATtiny85 `V_OH` is 2.5 V min at −5 mA and
  V_CC = 3 V (Table 21-1), and the AQY212EH is only *guaranteed* to turn on up to 3.0 mA
  of LED current. At 330 Ω on an aged 2.7 V cell that is 2.6 mA — under the bar. The
  failure mode is a box that quietly stops calibrating as its cell ages, which is the one
  failure a calibration monitor must not have. At 180 Ω it is 4.8 mA at end of life, and
  it costs ~6 mAh/year against a budget the watchdog dominates (sleep is **10 µA max** per
  datasheet, not the 5 µA assumed, so ~4 years typical rather than ~5).
- **The 22 µF does not hold the rail through a pulse.** 500 ms at 6.5 mA is 3.3 mC; the
  cap holds 66 µC, 50× short. The *cell* supplies the burst through its own ESR, so cell
  A's life is set by ESR rather than capacity — worst case being the shunted burst, where
  both LEDs are lit at once. Practical upshot for the acceptance test: **if bursts go
  flaky, suspect the cell before the firmware.**

**Two open questions closed, with numbers rather than opinions.** "Injector mid-cable vs.
shunt at the board end" was never a conflict — `rev2-frontend.md:233` gives cable
resistance as its reason, and 0.5–1 m is a fraction of an ohm against 375, under 0.3 % of
ζ. And the 249 kΩ + LM4040 path that sits across the coil permanently costs 0.15 % of
signal and contributes 0.1 nV/√Hz. **But it is open at the holder with cell B out**, so
it is stage 2 of bring-up that tests it, not stage 1 as the BOM claimed — a narrowing of
that claim, not a change to the procedure.

**The box: 138 × 78 × 45 mm, and not one of those is a preference.** 45 tall because a
D-series flange needs a 38 mm pad and a pad needs a wall to sit on; 138 long because two
XLRs facing each other eat 2 × 32 mm before the board gets any interior; 78 wide because
the 1/4" jack and the button intrude ~20 mm, which is why the board is offset to −Y —
which in turn leaves the −Y wall free for the cell holders. Tray plus flat lid, so every
cutout is in the tray and the piece you reprint is never the one you already got right.
The XLR cutout is `xlr_coupon.py`'s validated geometry reused, not re-derived.

**Verified by probing the exported mesh, not by trusting the volume:** watertight,
90.6 cm³, and every bore confirmed open by point containment. A watertight solid of the
right volume can still have a blind hole.

**Not validated, and flagged in red on the drawing:** the 1/4" jack bore (10.0, derived
from the 3/8"-32 nominal) and the button bore (12.0, assumed). Both are drillable in 3 mm
of PLA behind a ~14 mm flange — but **confirm the button's actual thread before printing**,
because a 16 mm button is a reprint rather than a drill.
