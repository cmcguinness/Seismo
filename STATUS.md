...
- **Gen-1 geophone case (2026-07-28) — modeled, not yet printed.** `parts/geophone_case.py` + `parts/geophone_case_lid.py`. A deliberately crude POC whose only jobs are *pick it up and set it down* and *put the XLR on the case so the cable unplugs*. **116 × 116 × 57 mm** rounded square, PLA, no seals, no ballast, no inserts; 9 × #6 sheet-metal screws total. Three screw heads as feet (three-point contact; also lets the floor print flat on the bed instead of bridging over printed feet). Six wall vents, none in the lid. The XLR **24 mm bore is modeled but its two M3 flange holes are not** — spacing unconfirmed, so mark through the connector and hand-drill 2.5 mm rather than risk scrapping the print. +Y wall locally thinned to 2.4 mm (D-series accepts a 1–3 mm panel). Views: `doc/geophone_case*.svg`.
