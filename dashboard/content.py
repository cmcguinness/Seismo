...
CALIBRATION_SECTIONS = [
    ...
    ("Why not just read the datasheet",
     ...
     'sheet claims.</p>'
     '<p class="mb-0 prose"><b>It is not a wiring fault, and that is worth saying '
     'precisely, because a factor of three is far too big to shrug at.</b> To lose that '
     'much signal in the electronics you would need something around 170&nbsp;&#8486; '
     'sitting across the coil. There is nothing like it: the damping socket on the '
     'interface board is empty, the bias resistors are 100&nbsp;k&#8486; &mdash; a loss '
     'of about half a percent &mdash; and the digitiser\'s input impedance is orders of '
     'magnitude too high to matter. No divider, no fault.</p>'
     '<p class="mb-0 prose">Nor is it a weak coil. A geophone\'s output and its coil '
     'resistance rise together, roughly as the square root, so an element genuinely '
     'three times quieter should measure nearer 40&nbsp;&#8486; than 375. This one '
     'measures <b>385&nbsp;&#8486;</b> against a 375&nbsp;&#8486; specification. The '
     'winding is what it claims to be.</p>'
     '<p class="mb-0 prose">So the factor of three is real and still unexplained. Three '
     'candidates remain, and the order matters. <b>The two sites are not the same '
     'ground</b> &mdash; the reference is {refdist}&nbsp;km away, and local conditions '
     'at these frequencies can differ by a factor of two on their own. <b>The '
     'measurement leans on the two numbers this page admits are guesses</b>: the '
     'comparison assumes 5&ndash;15&nbsp;Hz is safely above the corner, which is only '
     'true if the corner really is 4.5&nbsp;Hz. And <b>a copy can match the winding and '
     'miss the magnet.</b> Every one of those is answered by putting a known current '
     'through the coil, which is what the box in the next section is for.</p>'
     '<p class="mb-0 prose">Meanwhile the practical consequence stands: every magnitude '
     'this station reported before that measurement was out by that factor.</p>'),
